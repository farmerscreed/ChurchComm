...
import { AlertTriangle, Loader2, ChevronsRight, User, MessageSquare } from 'lucide-react';
                                ...
                                selectedFollowUp.notes.map((note, index) => (
                                    <div key={index} className="text-sm">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">
                                            {note.type === 'system' ? 'System' : (note.user_name || 'User')} ΓÇó {formatDistanceToNow(parseISO(note.timestamp), { addSuffix: true })}
                                        </p>
                                    </div>
                                ))
                            ) : (
