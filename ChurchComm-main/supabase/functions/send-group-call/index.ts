...
serve(async (req) => {
  ...
  try {
    ...
    const VAPI_PHONE_NUMBER_ID_DEFAULT = Deno.env.get('VAPI_PHONE_NUMBER_ID')

    if (!VAPI_API_KEY || !VAPI_PHONE_NUMBER_ID_DEFAULT) {
      console.error('Vapi configuration incomplete')
      throw new Error('Vapi configuration incomplete')
    }

    // Check if org has a dedicated phone number
    const { data: org } = await supabaseAdmin
      .from('organizations')
      .select('dedicated_phone_number, phone_number_type')
      .eq('id', organizationId)
      .single()

    const VAPI_PHONE_NUMBER_ID = (org?.phone_number_type === 'dedicated' && org?.dedicated_phone_number)
      ? org.dedicated_phone_number
      : VAPI_PHONE_NUMBER_ID_DEFAULT

    console.log('VAPI Config - API Key present:', !!VAPI_API_KEY, 'Phone ID:', VAPI_PHONE_NUMBER_ID, 'Type:', org?.phone_number_type || 'shared')
    ...
    if (isIndividualCall) {
      ...
      const { data: person, error: personError } = await supabaseAdmin
        .from('people')
        .select('id, first_name, last_name, phone_number, do_not_call')
        .eq('id', recipientId)
        .single()
      ...
      if (person?.do_not_call) {
        return new Response(JSON.stringify({
          error: 'This person has opted out of calls (do_not_call flag is set)'
        }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 400,
        })
      ...
      // Get all phone numbers from the group, excluding do_not_call
      ...
      recipients = members
        ?.map(m => m.people)
        .filter(person => person.phone_number && !person.do_not_call) || []
    }
