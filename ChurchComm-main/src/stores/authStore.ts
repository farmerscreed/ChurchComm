...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  signOut: async () => {
    ...
    try {
      ...
      set({ user: null, organization: null, currentOrganization: null, currentMember: null });
    } catch (error: any) {
  ...
  fetchOrganizations: async () => {
    ...
    try {
      ...
      // Store the current member record (includes role)
      ...
      set({ currentMember: memberRecord });
  ...
  hasPermission: (action: string, subject: string) => {
    const { user, currentOrganization, currentMember } = get();
    ...
    const permissions: Record<string, Record<string, string[]>> = {
      pastor: {
        'read': ['people', 'campaigns', 'call_history', 'groups', 'scripts', 'escalations'],
        'write': ['people', 'campaigns', 'scripts'],
        'manage': ['people', 'campaigns', 'scripts', 'escalations'],
      },
      member: {
        'read': ['people', 'campaigns', 'call_history', 'groups'],
        'write': [],
      },
    };
