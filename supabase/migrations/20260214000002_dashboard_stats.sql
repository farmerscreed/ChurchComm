...
BEGIN
  ...
  SELECT
    ...
    (SELECT COALESCE(SUM(call_duration), 0)::double precision / 60.0 FROM vapi_call_logs WHERE organization_id = p_organization_id) as total_minutes,
    (SELECT COUNT(*) FROM calling_campaigns WHERE organization_id = p_organization_id AND status = 'active') as active_campaigns,
    (SELECT COUNT(*) FROM vapi_call_logs WHERE organization_id = p_organization_id AND escalation_priority IS NOT NULL AND needs_pastoral_care = true AND escalation_status = 'open') as open_escalations;
END;
$$;
